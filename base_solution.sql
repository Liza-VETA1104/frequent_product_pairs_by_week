...
Особенности:
- Используется self-join для генерации уникальных пар (product_a < product_b)
- Учитывается год + неделя (ISO), чтобы избежать коллизий между годами
- Для производительности рекомендуется предварительно отфильтровать:
    * только популярные товары (топ-N)
    * или чеки разумного размера

Поддерживаемые СУБД: PostgreSQL, BigQuery
*/
